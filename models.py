...
class DApredictModel(nn.Module):
    ...
    def forward(self, X_da, Y_da, X_utt, Y_utt, step_size, turn,
        ...
        if config['use_utt']:
            ...
            dec_hidden = torch.cat((da_context_output, utt_context_output, turn), dim=2) # (batch_size, 1, DEC_HIDDEN)
        else:
    ...
    def evaluate(self, X_da, Y_da, X_utt, Y_utt, turn,
        ...
        if config['use_utt']:
            ...
            dec_hidden = torch.cat((da_context_output, utt_context_output, turn), dim=2) # (1, 1, DEC_HIDDEN)
        else:
            dec_hidden = torch.cat((da_context_output, turn), dim=2)
